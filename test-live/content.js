...
function setInitialStyles() {
  const { ringWidth: highlightRingWidth, ringColor: highlightRingColor } =
    getHighlightRingConfig();
  initialCSStyles = `
  #woAcc-Modal h1,
  #woAcc-Modal h2,
  #woAcc-Modal h3,
  ...
  .woAcc-FLF1 > :not(#woAcc-RootEle) form {
    outline: ${highlightRingWidth}px solid ${highlightRingColor} !important;
    outline-offset: -${highlightRingWidth}px !important;
    box-shadow: inset 0 0 0 ${highlightRingWidth}px ${highlightRingColor} !important;
  }
  ...
  /* The :not(#woAcc-NeverMatch):not(#woAcc-NeverMatch) selectors match every
     element (no element carries that id) while adding two id-levels of
     specificity, so the widget cursor also wins against host-site rules like
     "a:hover { cursor: pointer !important; }". */
  .woAcc-FCUR1-black,
  .woAcc-FCUR1-black *,
  .woAcc-FCUR1-black [type="submit"],
  .woAcc-FCUR1-black a,
  .woAcc-FCUR1-black :not(#woAcc-NeverMatch):not(#woAcc-NeverMatch) {
  ...
  .woAcc-FCUR1-white a,
  .woAcc-FCUR1-white :not(#woAcc-NeverMatch):not(#woAcc-NeverMatch) {
  ...
  .woAcc-FCURSOR-custom :not(#woAcc-NeverMatch):not(#woAcc-NeverMatch) {
...
/**
 * Highlight rings (linkHighlight, highlightTitles, highlightForm,
 * highlightHover, keyboard-navigation focus).
 *
 * The rings are applied as inline styles with the 'important' priority in
 * addition to the stylesheet rules, because inline !important declarations
 * beat host-page stylesheet !important rules — even rules living inside CSS
 * cascade layers (e.g. Tailwind v4 sites shipping
 * `* { outline: none !important; box-shadow: none !important; }` in
 * `@layer base`, which beats any unlayered stylesheet rule of ours).
 * The negative outline-offset draws the ring INSIDE the border box so
 * ancestors with overflow/clip-path cannot clip it, and the inset box-shadow
 * is a redundant ring for hosts that only reset outlines. Neither property
 * affects layout, so elements never move.
 */
const highlightRingSelectors = {
  linkHighlight: 'a',
  highlightTitles: 'h1, h2, h3, h4, h5, h6',
  highlightForm: 'form',
};
const inlineRingProperties = ['outline', 'outline-offset', 'box-shadow'];
const activeHighlightRings = {};
const inlineRingOriginalStyles = new WeakMap();
let highlightRingObserver = null;

function getHighlightRingConfig() {
  return {
    ringWidth: Math.max(1, Number(visuals.highlight.highlightSize) || 2),
    ringColor: hexToRgba(
...
function saveInlineRingOriginal(element) {
  if (inlineRingOriginalStyles.has(element)) {
    return;
  }
  const original = {};
  inlineRingProperties.forEach((property) => {
    original[property] = [
      element.style.getPropertyValue(property),
      element.style.getPropertyPriority(property),
    ];
  });
  inlineRingOriginalStyles.set(element, original);
}

function restoreInlineRingOriginal(element) {
  const original = inlineRingOriginalStyles.get(element);
  inlineRingProperties.forEach((property) => {
  ...
  inlineRingOriginalStyles.delete(element);
}

function applyHighlightRingStyles(element) {
  const { ringWidth, ringColor } = getHighlightRingConfig();
  saveInlineRingOriginal(element);
  element.style.setProperty(
    'outline',
    `${ringColor} solid ${ringWidth}px`,
    'important'
  );
  element.style.setProperty('outline-offset', `-${ringWidth}px`, 'important');
  element.style.setProperty(
    'box-shadow',
...
function isElementHighlightRinged(element) {
  return Object.keys(activeHighlightRings).some((fieldId) =>
    activeHighlightRings[fieldId].has(element)
  );
}

function syncHighlightRingObserver() {
  const hasActiveRings = Object.keys(activeHighlightRings).length > 0;
  if (hasActiveRings && !highlightRingObserver) {
    highlightRingObserver = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType !== Node.ELEMENT_NODE) {
            return;
          }
          if (node.closest?.('#woAcc-RootEle')) {
            return;
          }
          Object.keys(activeHighlightRings).forEach((fieldId) => {
            const ringedElements = activeHighlightRings[fieldId];
            const selector = highlightRingSelectors[fieldId];
            if (node.matches?.(selector)) {
              applyHighlightRingStyles(node);
              ringedElements.add(node);
            }
            node.querySelectorAll?.(selector)?.forEach((element) => {
              applyHighlightRingStyles(element);
              ringedElements.add(element);
            });
          });
        });
      });
    });
    highlightRingObserver.observe(document.body, {
      childList: true,
      subtree: true,
    });
  } else if (!hasActiveRings && highlightRingObserver) {
    highlightRingObserver.disconnect();
    highlightRingObserver = null;
  }
}

function handleHighlightRing(fieldId, newKey) {
  const previouslyRinged = activeHighlightRings[fieldId];
  if (previouslyRinged) {
    delete activeHighlightRings[fieldId];
    previouslyRinged.forEach((element) => {
      if (
        !isElementHighlightRinged(element) &&
        !highlightHoverTag.has(element)
      ) {
        restoreInlineRingOriginal(element);
      }
    });
  }
  if (newKey === fieldId) {
    const ringedElements = new Set();
    document
      .querySelectorAll(highlightRingSelectors[fieldId])
      .forEach((element) => {
    ...
    activeHighlightRings[fieldId] = ringedElements;
  }
  syncHighlightRingObserver();
}

function refreshActiveHighlightRings() {
  Object.keys(activeHighlightRings).forEach((fieldId) => {
    activeHighlightRings[fieldId].forEach((element) => {
      applyHighlightRingStyles(element);
    });
  });
...
function setGuidyFocusRing(element) {
  element.classList.add('guidy-focused');
  saveInlineRingOriginal(element);
  element.style.setProperty('outline', '2.5px solid #0038ff', 'important');
  element.style.setProperty('outline-offset', '-2.5px', 'important');
  element.style.setProperty(
    'box-shadow',
    '0 0 0 8px rgba(0, 56, 255, 0.42)',
    'important'
  );
}

function clearGuidyFocusRing() {
  const element = document.querySelector('.guidy-focused');
  if (!element) {
    return;
  }
  element.classList.remove('guidy-focused');
  if (isElementHighlightRinged(element) || highlightHoverTag.has(element)) {
    applyHighlightRingStyles(element);
  } else {
    restoreInlineRingOriginal(element);
  }
...
function handleHighlightHover(newKey) {
  ...
  highlightHoverTag.forEach((element) => {
    if (!isElementHighlightRinged(element)) {
      restoreInlineRingOriginal(element);
    }
  });
...
async function changeClassName(fieldId, newKey, isInitial) {
  ...
  // }
    ...
    if (newKey === 'screenReader') {
      ...
      // isInitial = restoring from persisted state on page load: re-attach the
      // read handlers but do NOT auto-announce / read the whole page (autoplay
      // would block it and it is poor UX). Speech resumes on user interaction.
      startScreenReader(isInitial);
    }
    ...
    if (newKey !== 'default') {
      ...
      // isInitial = restoring on page load: re-attach the click/select read
      // handlers without the initial "click where you want to read" prompt.
      startPartialScreenReader(isInitial);
    } else {
      currentReadSpeed = 'moderateSpeed';
    }
    ...
    fieldId === 'linkHighlight' ||
    fieldId === 'highlightTitles' ||
    fieldId === 'highlightForm'
  ) {
    handleHighlightRing(fieldId, newKey);
...
function focusFirstVisibleElement() {
  ...
  if (firstVisibleElement) {
    setGuidyFocusRing(firstVisibleElement);
...
function getCursorSVG(key, size = visuals.cursorIcon.cursorSize) {
  const cursorSVGs = {
    hand: `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 36 36" fill="${visuals.cursorIcon.cursorColor}">
  ...
  <path d="M17.2148 19.4219L17.2519 25.4678" stroke="currentColor" stroke-width="0.75" stroke-linecap="round"/>
    ...
    italic: `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 36 36" fill="${visuals.cursorIcon.cursorColor}">
  ...
  <path d="M17.333 29C16.9758 29.9607 15.5028 31.4338 14.8469 31.8346C14.0625 32.3104 13.1918 32.6178 12.3887 32.8173C11.4036 33.0643 9.33301 32.9893 9.33301 32.9893" stroke="#currentColor"/>
    ...
    bigCursor: `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 36 36" fill="${visuals.cursorIcon.cursorColor}">
...
// Visual tip of each cursor glyph inside its 36x36 SVG viewBox. The CSS
// cursor hotspot must sit on this point (scaled to the rendered size) so
// hover/click happens exactly where the user sees the cursor point at every
// size. 'italic' is an I-beam whose native convention is a centered hotspot.
const CURSOR_VIEWBOX_SIZE = 36;
const customCursorTips = {
  default: { x: 0, y: 0 },
  bigCursor: { x: 8.1, y: 3 },
  hand: { x: 11.5, y: 4 },
  italic: { x: 18.3, y: 18 },
...
function createCustomCursor(newKey) {
  if (newKey && newKey !== 'default') {
    // Browsers ignore cursor images larger than ~128 device pixels (the
    // cursor silently falls back to the native one), so cap the rendered
    // size accordingly on scaled displays.
    const requestedSize = Number(visuals.cursorIcon.cursorSize) || 32;
    const maxSize = Math.floor(128 / (window.devicePixelRatio || 1));
    const size = Math.min(requestedSize, maxSize);
    const svg = getCursorSVG(newKey, size);
    const encodedSvg = encodeURIComponent(svg);
    const cursorUrl = `data:image/svg+xml,${encodedSvg}`;
    const tip = customCursorTips[newKey] || customCursorTips.default;
    const clampToImage = (value) =>
      Math.max(
        0,
        Math.min(size - 1, Math.round((value / CURSOR_VIEWBOX_SIZE) * size))
      );
    const hotspotX = clampToImage(tip.x);
    const hotspotY = clampToImage(tip.y);
...
function handleVisuals(key, fieldId, newKey) {
  ...
  if (layeredStyleElement) {
    layeredStyleElement.innerHTML = `@layer woaccwidget {${initialCSStyles}}`;
  }
}
...
let shortcutFeedbackToast;
let shortcutFeedbackToastTimer;

function ensureShortcutFeedbackToast() {
  if (!shortcutFeedbackToast) {
    shortcutFeedbackToast = document.createElement('div');
    shortcutFeedbackToast.classList.add('woAcc-shortcutToast');
    shortcutFeedbackToast.setAttribute('role', 'status');
    shortcutFeedbackToast.setAttribute('aria-live', 'polite');
    shortcutFeedbackToast.setAttribute('aria-atomic', 'true');
  }
  if (!shortcutFeedbackToast.isConnected) {
    rootElement.appendChild(shortcutFeedbackToast);
  }
  return shortcutFeedbackToast;
}

function showShortcutFeedbackToast(label, value) {
  const toast = ensureShortcutFeedbackToast();
  const stateKey = value === 'default' ? 'Off' : 'On';
  toast.textContent = `${getTranslations(label)}: ${getTranslations(stateKey)}`;
  toast.classList.add('woAcc-shortcutToast-visible');
  clearTimeout(shortcutFeedbackToastTimer);
  shortcutFeedbackToastTimer = setTimeout(() => {
    toast.classList.remove('woAcc-shortcutToast-visible');
    toast.textContent = '';
  }, 2500);
}
  ...
  if (!isEditing) {
    ...
    if (shortCut) {
      ...
      if (option && options.length) {
        ...
        showShortcutFeedbackToast(shortCut.label, nextValue);
        e.stopPropagation();
      }
    }
  }
...
function handleVirtualKeyboard(newKey) {
  ...
  if (newKey !== 'default') {
    ...
    // Open the keyboard in the widget's language: Japanese (kana) mode when the
    // widget language is Japanese, English otherwise. Users can still switch
    // with the あいう / ABC keys. Re-seeded on each enable so a widget language
    // change is reflected the next time the keyboard is turned on.
    isShifted = false;
    isJapanese = getDataById('woAccessibilityLang') === 'ja';
    createBoard();
...
export function setDataById(id, updatedData) {
  ...
  if (
    // screenReader/partialReader were previously excluded because enabling them
    // auto-speaks (announcement + read); persisting + restoring them on load
    // is now safe because the load-time restore path passes isInitial=true,
    // which re-attaches the read handlers WITHOUT the initial speech attempt
    // (see changeClassName/startScreenReader/startPartialScreenReader below).
    !['pageStructure', 'readingMode', 'personalizeYourShortcut'].includes(id)
  ) {
...
export function handleContentStateChange(name, currentActive, isInitial) {
  setDataById(name, currentActive);
  changeClassName(name, currentActive, isInitial);
}
...
function setInitialThings() {
  ...
  // Cascade-layer hardening: declare the widget layer as the FIRST node of
  // <head> so it is the earliest-declared layer in document order (position,
  // not insertion time, decides layer order). For !important declarations the
  // earliest layer wins, so the layered copy below beats host-page layered
  // !important rules (e.g. Tailwind v4 `@layer base` resets) that would
  // otherwise override our unlayered stylesheet. The unlayered styleElement
  // above is kept as-is: browsers without @layer support ignore the layered
  // copy entirely and keep today's behavior, and normal (non-important)
  // declarations keep their current unlayered strength.
  const layerStatementElement = document.createElement('style');
  layerStatementElement.innerHTML = '@layer woaccwidget;';
  document.head.insertBefore(layerStatementElement, document.head.firstChild);
  layeredStyleElement = document.createElement('style');
  layeredStyleElement.innerHTML = `@layer woaccwidget {${initialCSStyles}}`;
  document.head.appendChild(layeredStyleElement);
...
function startScreenReader(isInitial) {
  const onClick = (e) => {
  ...
  // On a fresh page-load restore (isInitial), skip the announcement and the
  // automatic whole-page read: there is no user gesture yet, so the browser
  // would block the audio, and auto-reading the entire page is undesirable.
  // The click/keyup handlers below still make the reader active — speech then
  // happens in response to the user's first interaction.
  if (!isInitial) {
...
function startPartialScreenReader(isInitial) {
  // On a fresh page-load restore (isInitial) skip the spoken prompt: no user
  // gesture yet, so the audio would be blocked. The click/keyup handlers below
  // still make the selected-text reader active for the user's next click.
  if (!isInitial) {
    say(
