...
import {
  ...
  resolveRules,
  watchNavigationForRuleFixes,
} from './ruleResolver.js';
...
function checkAuth() {
  const scripts = Array.from(
    document.querySelectorAll('script[data-account]')
  ).filter(
    (e) => e.src.includes('injector.js') || e.src.includes('accessibility.js')
  );

  if (scripts.length === 0) return;

  const accountKeys = Array.from(
    new Set(scripts.map((s) => s.dataset.account).filter(Boolean))
  );

  if (accountKeys.length === 0) return;

  console.log('Found account keys:', accountKeys);

  let isLoaded = false;

  const tryVerify = (index) => {
    if (index >= accountKeys.length || isLoaded) return;
    const accountKey = accountKeys[index];
    console.log(`Attempting verification for account key: ${accountKey}`);
    ...
    if (
      ...
      verifySubscription(accountKey)
        .then((config) => {
          ...
          const guidyScript = scripts.find(
            (s) => s.dataset.account === accountKey
          );
          if (config) {
          ...
          resolveRules();
          // Fixes are served per page — re-resolve them on SPA route changes.
          watchNavigationForRuleFixes();
          injectSkipButtonDiv();
          injectCommonStyles();
        ...
        .catch((err) => {
          console.warn(`Failed to verify account key: ${accountKey}`, err);
