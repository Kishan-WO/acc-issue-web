...
// Fixes are served per page (origin + pathname); track the key we last
// resolved so SPA navigation only re-fetches when the page actually changed.
let lastResolvedPageKey = null;
let navigationWatcherInstalled = false;

function getCurrentPageKey() {
  // Match the server's page-key normalization (trailing slashes stripped) so
  // slash-toggling replaceState calls do not trigger redundant re-resolves.
...
/**
 * Re-applies page fixes after SPA client-side navigation (pushState /
 * replaceState / popstate). Hash- or query-only changes share the same page
 * key and are skipped. resolveRules runs twice (fast + late pass) because
 * SPA route content mounts after the history event. Re-running is safe: the
 * attribute-setting resolvers are idempotent and the element-replacing /
 * injecting resolvers carry re-entry guards.
 * Called from injector.js right after the initial resolveRules().
 */
export function watchNavigationForRuleFixes() {
  if (navigationWatcherInstalled) return;
  navigationWatcherInstalled = true;

  const reResolveOnRouteChange = () => {
    // The dashboard authoring iframe drives fix state itself.
    if (window.guidyDashboardFixMode) return;
    if (getCurrentPageKey() === lastResolvedPageKey) return;
    setTimeout(resolveRules, 100);
    setTimeout(resolveRules, 1500);
  };

  const originalPushState = history.pushState;
  history.pushState = function (...args) {
    const result = originalPushState.apply(this, args);
    reResolveOnRouteChange();
    return result;
  };
  ...
  window.addEventListener('popstate', reResolveOnRouteChange);
}
...
export async function resolveRules() {
  try {
    ...
    lastResolvedPageKey = getCurrentPageKey();
    ...
    if (response.ok) {
      ...
      rules.forEach((rule) => {
        // One malformed rule must not abort the remaining rules on the page.
        try {
          const ruleResolver = resolver[rule?.id];
          ruleResolver?.(rule?.selectors);
        } catch (ruleError) {
          console.log(`Error resolving rule "${rule?.id}":`, ruleError);
        }
      });
    }
    // Always mark resolution complete (even on non-ok responses) — the
    // accessibility scanner waits on this attribute before running axe.
...
function fixAriaHiddenElement(element, isChild = false) {
  try {
    ...
    // Keep previously saved originals on re-runs (SPA navigation) so a later
    // revert can still restore the pre-fix attribute values.
    element.guidyAttrValues = element.guidyAttrValues || [];
    if (element) {
      if (isChild) {
...
const observeAttributeChanges = (elements) => {
  elements.forEach((element) => {
    // Already watched (e.g. rules re-resolved after SPA navigation) — do not
    // stack another observer on the same element.
    if (element.guidyMutationObserverId !== undefined) return;
    const observer = new MutationObserver((mutations) => {
      // Element left the DOM (SPA navigation) — stop observing it.
...
function resolvePAsHeading(selectors) {
  ...
  filterSelectors.forEach((selector) => {
    ...
    if (!element || !newTagName) return;
    // Already the desired heading tag (e.g. rules re-resolved after SPA
    // navigation matched the replaced element) — nothing to do.
    if (element.tagName.toLowerCase() === newTagName.toLowerCase()) return;
    const newElement = document.createElement(newTagName);
...
function resolveHasMultipleH1(selectors) {
  ...
  for (const [key, value] of Object.entries(userInput || {})) {
    ...
    if (element && value !== 'h1') {
      ...
      // Already the desired tag (re-run after SPA navigation) — skip.
      if (element.tagName.toLowerCase() === newTagName.toLowerCase()) continue;
      const newElement = document.createElement(newTagName);
...
function applyValidations(formEl, inputEl, validations) {
  // Re-runs (SPA navigation) must not stack duplicate listeners; the
  // dashboard preview is exempt because it re-applies changed validations.
  if (!window.guidyDashboardFixMode && inputEl.guidyValidationApplied) return;
  inputEl.guidyValidationApplied = true;
  const validators = {
...
function resolveSectionHeadings(selectors = []) {
  ...
  filterSelectors.forEach((selector) => {
    ...
    if (element) {
      ...
      const attributes = userInput?.attributes;
      if (tagName && content) {
        // Heading already injected (re-run after SPA navigation) — skip so
        // the section does not accumulate duplicates.
        if (element.querySelector(':scope > [data-guidy-section-heading]')) {
          return;
        }
        const newElement = document.createElement(tagName);
        newElement.setAttribute('data-guidy-section-heading', 'true');
        const firstKindOfElem = document.querySelector(tagName);
        const classList = firstKindOfElem?.classList || [];
        newElement.style.textAlign = 'center';
        classList.forEach((cls) => {
